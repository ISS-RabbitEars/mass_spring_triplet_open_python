...
dR = np.asarray([sp.sqrt((X[i+1] - X[i])**2 + (Y[i+1] - Y[i])**2) for i in range(2)])
...
m = np.array([1, 1, 1])
k = np.array([25, 25])
req = np.array([2.25, 2.25])
ro = np.array([1.5, 2.6, 1.5])
vo = np.array([0, 0, 0])
thetao = np.array([0, 90, 180]) * np.pi/180
omegao = np.array([0, 0, 0]) * np.pi/180
tf = 30 
...
dr = np.asarray([np.sqrt((x[i+1] - x[i])**2 + (y[i+1] - y[i])**2) for i in range(2)])
rmax = np.asarray([max(dr[i]) for i in range(2)])
theta = np.asarray([np.arccos((y[i+1] - y[i])/dr[i]) for i in range(2)])
nl = np.asarray([np.ceil(rmax[i]/(mra[i]+mra[i+1])) for i in range(2)],dtype=int)
l = np.asarray([(dr[i] - (mra[i]+mra[i+1]))/nl[i] for i in range(2)])
h = np.asarray([np.sqrt(((mra[i]+mra[i+1])/2)**2 - (0.5 * l[i])**2) for i in range(2)])
flipa = np.zeros((2,nframes))
flipb = np.zeros((2,nframes))
flipc = np.zeros((2,nframes))
for i in range(2):
...
xlo = np.zeros((2,nframes))
ylo = np.zeros((2,nframes))
for i in range(2):
	xlo[i] = x[i] + np.sign((y[i+1] - y[i]) * flipa[i] * flipb[i]) * mra[i] * np.sin(theta[i])
	ylo[i] = y[i] + mra[i] * np.cos(theta[i])
xl = np.zeros((2,max(nl),nframes))
yl = np.zeros((2,max(nl),nframes))
for i in range(2):
	for j in range(nl[i]):
...
xlf = np.zeros((2,nframes))
ylf = np.zeros((2,nframes))
for i in range(2):
...
def run(frame):
	...
	plt.title("Mass-Spring Triplet, Open Ended")
	ax=plt.gca()
	ax.set_aspect(1)
...
ani.save('mass_spring_triplet_open.mp4', writer=writervideo)
plt.show()
